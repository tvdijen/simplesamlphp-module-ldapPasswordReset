...
$t->addTemplatesFromModule('ldapPasswordReset');
/*
$path = dirname(dirname(__FILE__)) . '/templates/' . $template;
        list($namespace, $filename) = function (string $path) {
            $tmp = explode(':', $path, 2);
            return (count($tmp) === 2) ? [$tmp[0], $tmp[1]] : [null, $tmp[0]];
        };
*/
        list($namespace, $filename) = $this->findModuleAndTemplateName(dirname(dirname(__FILE__)) . '/templates/' . $template);
        $twigTemplate = ($namespace !== null) ? '@' . $namespace . '/' . $filename : $filename;
/*
        $t->data = [
...
var_dump($t->getTwig()->getLoader());
        $result = $t->getTwig()->render($twigTemplate, [
            'subject' => $this->mail->Subject,
            'text' => $this->text,
            'data' => $this->data
        ]);
